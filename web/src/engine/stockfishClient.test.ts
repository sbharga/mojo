...
import { StockfishClient, type UciWorker } from './stockfishClient'
...
describe('StockfishClient', () => {
  it('initializes UCI and starts a configured search with full history', () => {
    ...
    expect(worker.messages).toContain('position fen root fen moves e2e4 e7e5')
  ...
  it('reports a missing move and terminates cleanly', () => {
    ...
    expect(callbacks.onError).toHaveBeenCalledWith('Stockfish did not return a legal move')
