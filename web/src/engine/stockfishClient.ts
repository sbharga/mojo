...
export class StockfishClient {
  ...
  private schedule() {
    if (!this.initialized || this.current || this.preparing || !this.pending) return
    this.preparing = this.pending
    this.pending = null
    const elo = Math.round(clamp(this.preparing.request.elo, MIN_STOCKFISH_ELO, MAX_STOCKFISH_ELO))
  ...
  private handleLine(line: string) {
    ...
    if (line === 'readyok') {
      ...
      this.current = search
      const moves = search.request.moves.length ? ` moves ${search.request.moves.join(' ')}` : ''
