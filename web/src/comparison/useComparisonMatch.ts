...
function moduleUrl(version: EngineVersion) {
  return new URL(
    `${import.meta.env.BASE_URL}engine-versions/${version.modulePath}`,
...
export function useComparisonMatch() {
  ...
  const start = useCallback(
    (configuration: MatchConfiguration, openingSuite: Opening[]) => {
      terminate()
      ...
      const instances = Array.from({ length: workerCount }, () => {
        ...
        worker.onmessage = (event: MessageEvent<ComparisonWorkerMessage>) => {
          ...
          if (message.type === 'error') {
            fail(
              `${message.pairIndex === undefined ? 'Engine initialization' : `Opening pair ${message.pairIndex + 1}`} failed: ${message.message}`,
            )
            return
          }
  ...
  const exportData: MatchExport | null =
    state.configuration && state.startedAt && state.finishedAt
      ? {
          status:
