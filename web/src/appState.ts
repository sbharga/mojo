...
export const MIN_THINK_TIME_MS = 100
export const MAX_THINK_TIME_MS = 10_000
...
export function loadSettings(storage: StorageReader): typeof DEFAULT_SETTINGS {
  try {
    ...
    return {
      mode:
        value.mode === 'human-engine' ||
        value.mode === 'human-stockfish' ||
        value.mode === 'engine-engine' ||
        value.mode === 'mojo-stockfish' ||
        value.mode === 'human-human'
          ? value.mode
          : DEFAULT_SETTINGS.mode,
      humanSide:
        value.humanSide === 'white' || value.humanSide === 'black'
          ? value.humanSide
          : DEFAULT_SETTINGS.humanSide,
      thinkTime:
        typeof value.thinkTime === 'number' && Number.isFinite(value.thinkTime)
          ? Math.min(MAX_THINK_TIME_MS, Math.max(MIN_THINK_TIME_MS, value.thinkTime))
          : DEFAULT_SETTINGS.thinkTime,
      stockfishElo:
        typeof value.stockfishElo === 'number' && Number.isFinite(value.stockfishElo)
          ? Math.round(Math.min(MAX_STOCKFISH_ELO, Math.max(MIN_STOCKFISH_ELO, value.stockfishElo)))
          : DEFAULT_SETTINGS.stockfishElo,
      stockfishThinkTime:
        typeof value.stockfishThinkTime === 'number' && Number.isFinite(value.stockfishThinkTime)
          ? Math.min(MAX_THINK_TIME_MS, Math.max(MIN_THINK_TIME_MS, value.stockfishThinkTime))
          : DEFAULT_SETTINGS.stockfishThinkTime,
      stockfishSide:
        value.stockfishSide === 'white' || value.stockfishSide === 'black'
          ? value.stockfishSide
          : DEFAULT_SETTINGS.stockfishSide,
      flipped: typeof value.flipped === 'boolean' ? value.flipped : DEFAULT_SETTINGS.flipped,
      showBestMove:
        typeof value.showBestMove === 'boolean'
          ? value.showBestMove
          : DEFAULT_SETTINGS.showBestMove,
    }
  } catch {
