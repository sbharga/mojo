...
import { formatUci, parseUci } from './engine/uci'
...
function App() {
  ...
  const applyEngineMove = useCallback(
    (uci: string) => {
      ...
      try {
        game.current.move(parseUci(uci))
        sync()
      } catch {
        /* A stale move is safely ignored. */
      }
    },
    [sync],
  )
  const {
    ...
    start,
    cancel,
  } = useEngine(applyEngineMove)
  const stockfishEnabled = mode === 'human-stockfish' || mode === 'mojo-stockfish'
  const {
    isReady: isStockfishReady,
    isThinking: stockfishThinking,
    error: stockfishError,
    start: startStockfish,
    cancel: cancelStockfish,
  } = useStockfish(stockfishEnabled, applyEngineMove)
  ...
  const positionHistory = useMemo(
    () => history.slice(0, viewPly).map((move) => move.before),
    [history, viewPly],
  )
  const stockfishMoves = useMemo(() => history.map(formatUci), [history])
  const mojoToMove =
    atLatest &&
    !gameOver &&
    ((mode === 'human-engine' && turn !== humanSide) ||
      (mode === 'engine-engine' && running) ||
      (mode === 'mojo-stockfish' && running && turn !== stockfishSide))
  const stockfishToMove =
    atLatest &&
    !gameOver &&
    ((mode === 'human-stockfish' && turn !== humanSide) ||
      (mode === 'mojo-stockfish' && running && turn === stockfishSide))
  const humanCanMove =
    !gameOver &&
    (!atLatest ||
      mode === 'human-human' ||
      ((mode === 'human-engine' || mode === 'human-stockfish') && turn === humanSide))
  ...
  useEffect(() => {
    ...
    history.forEach((move) =>
      savedGame.move({ from: move.from, to: move.to, promotion: move.promotion }),
    )
    saveSession(
      localStorage,
      {
  ...
  useEffect(() => {
    ...
    if (stockfishToMove)
      startStockfish({
        rootFen,
        moves: stockfishMoves,
        elo: stockfishElo,
        thinkTimeMs: stockfishThinkTime,
      })
  ...
  const newGame = useCallback(() => {
    ...
    setRootFen(initialFen)
    setRunning(false)
    sync()
  }, [cancel, cancelStockfish, sync])
  ...
  const play = (from: string, to: string, piece?: string) => {
    ...
    try {
      ...
      const promotion =
        pawn?.type === 'p' && (to.endsWith('1') || to.endsWith('8'))
          ? piece?.[1]?.toLowerCase()
          : undefined
  ...
  const navigate = (ply: number) => {
    ...
    history
      .slice(0, targetPly)
      .forEach((move) => position.move({ from: move.from, to: move.to, promotion: move.promotion }))
  ...
  const playAnalysisMove = (move: string) => {
    if (
      !analysis?.lines.some((line) => line.moves[0] === move) ||
      analysis.root_fen !== fen ||
    ...
    try {
      cancel()
      cancelStockfish()
      game.current.move(parseUci(move))
      sync()
    } catch {
      /* A stale analysis line is safely ignored. */
    }
  }
  const loadFen = (value: string) => {
    try {
      game.current = new Chess(value.trim())
      setRootFen(game.current.fen())
      sync()
      setDialog(null)
    } catch {
      window.alert('That FEN is not a legal standard-chess position.')
    }
  }
  const loadPgn = (value: string) => {
    try {
      const loaded = new Chess()
      loaded.loadPgn(value)
      game.current = loaded
      setRootFen(loaded.getHeaders().FEN ?? initialFen)
      sync()
      setDialog(null)
    } catch {
      window.alert('That PGN could not be loaded.')
    }
  }
  ...
  const legalSquareStyles = useMemo(() => {
    ...
    game.current.moves({ square: selectedSquare, verbose: true }).forEach((move) => {
      styles[move.to] = game.current.get(move.to)
        ? { boxShadow: 'inset 0 0 0 5px #f6f669b0' }
        : { background: 'radial-gradient(circle, #f6f669a8 0 19%, transparent 21%)' }
    })
    return styles
  ...
  const arrows = useMemo<Array<[Square, Square, string]>>(() => {
    ...
    return showBestMove && move
      ? [[move.slice(0, 2) as Square, move.slice(2, 4) as Square, '#f4bd2e']]
      : []
    ...
    <main className="app">
      <header>
        <div className="brand">
          ...
          <div>
            <h1>Mojo</h1>
            <p>Browser chess engine</p>
          </div>
        </div>
        <div className="header-actions">
          <a className="nav-button" href="#/compare">
            Compare
          </a>
          <div className="status" role="status" aria-live="polite">
            <i className={isReady && !error ? 'ready' : ''} aria-hidden="true" />
            {error ??
              (gameOver
                ? 'Game over'
      ...
      <div className="workspace">
        <div className="board-area">
          <EvaluationBar
            scoreCp={evalLine?.score_cp ?? null}
            mateIn={evalLine?.mate_in ?? null}
            result={gameResult}
          />
          <div className="board-shell" ref={boardShell}>
            <Chessboard
              id="mojo-board"
              position={fen}
              boardOrientation={orientation}
              onPieceDrop={play}
              onPieceClick={(_, square) => selectSquare(square as Square)}
              onPieceDragBegin={(_, square) => selectSquare(square as Square)}
              onSquareClick={(square) => clickSquare(square as Square)}
              arePiecesDraggable={humanCanMove}
              autoPromoteToQueen={false}
              customArrows={arrows}
              boardWidth={boardWidth}
              customDarkSquareStyle={{ backgroundColor: '#779556' }}
              customLightSquareStyle={{ backgroundColor: '#ebecd0' }}
              customSquareStyles={legalSquareStyles as never}
            />
          </div>
        </div>
        <aside style={{ '--sidebar-height': `${sidebarHeight}px` } as CSSProperties}>
          <AnalysisPanel analysis={analysis} onSelectMove={playAnalysisMove} />
          <MoveHistory history={history} currentPly={viewPly} onNavigate={navigate} />
        </aside>
      </div>
      {dialog === 'settings' && (
        <dialog
          ...
          onClick={(event) => {
            if (event.target === settingsDialogRef.current) closeSettings()
          }}
        >
          ...
          <SettingsPanel
            ...
            stockfishThinkTime={stockfishThinkTime}
            stockfishSide={stockfishSide}
            running={running}
            showBestMove={showBestMove}
            onMode={setMode}
            onSide={setHumanSide}
            onTime={setThinkTime}
            onStockfishElo={setStockfishElo}
            onStockfishTime={setStockfishThinkTime}
            onStockfishSide={setStockfishSide}
            onToggle={() => setRunning((value) => !value)}
            onFlip={() => setFlipped((value) => !value)}
            onShowBestMove={setShowBestMove}
            onReset={() => {
              newGame()
              closeSettings()
            }}
      ...
      {(dialog === 'fen' || dialog === 'pgn' || dialog === 'export') && (
        <SetupDialog
          title={
            dialog === 'fen'
              ? 'Load FEN position'
          ...
          initialValue={dialog === 'fen' ? fen : game.current.pgn()}
          onClose={() => setDialog(null)}
          onSubmit={dialog === 'fen' ? loadFen : dialog === 'pgn' ? loadPgn : () => setDialog(null)}
          submitLabel={dialog === 'export' ? 'Close' : 'Load'}
