  ...
  return (
    <section className="panel analysis-panel" aria-labelledby="analysis-heading">
      <div className="panel__heading">
        <span id="analysis-heading">Mojo analysis</span>
        <small>
          {analysis
            ? `depth ${analysis.depth} · ${Math.round(analysis.nodes / 1000)}k nodes`
            : 'warming up'}
        </small>
      </div>
        ...
        {lines.length ? (
          lines.map((line, rank) => (
            <button
              ...
              key={rank}
              disabled={!line.moves[0]}
              onClick={() => line.moves[0] && onSelectMove(line.moves[0])}
              title={line.moves[0] ? `Play ${line.moves[0]}` : undefined}
            >
              <strong>{formatAnalysisScore(line)}</strong>
              <span>{line.moves.join(' ')}</span>
            </button>
          ))
        ) : (
          <div className="empty">Analysis will appear here.</div>
        )}
      </div>
    </section>
  )
