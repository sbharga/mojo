...
export function MoveHistory({ history, currentPly, onNavigate }: Props) {
  ...
  return (
    <section className="panel move-panel" aria-labelledby="move-history-heading">
      <div className="panel__heading">
        <span id="move-history-heading">Move history</span>
      ...
      <div className="moves" ref={movesElement}>
        {rows.length ? (
          rows.map((row) => (
            <div className="move-row" key={row.number}>
              <span>{row.number}.</span>
              {row.white && row.whitePly !== undefined && (
                <button
                  type="button"
                  className={currentPly === row.whitePly ? 'active' : ''}
                  onClick={() => onNavigate(row.whitePly!)}
              ...
              {row.black && row.blackPly !== undefined && (
                <button
                  type="button"
                  className={currentPly === row.blackPly ? 'active' : ''}
                  onClick={() => onNavigate(row.blackPly!)}
                  aria-label={`Go to ${row.number}... ${row.black.san}`}
                >
                  {row.black.san}
                </button>
              )}
          ...
          <div className="empty">The game starts here.</div>
        )}
      </div>
    </section>
  )
