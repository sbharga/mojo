...
export function EvaluationBar({ scoreCp, mateIn, result = null }: Props) {
  ...
  const whiteAhead =
    finished && result !== 'draw' ? result === 'white' : hasMate ? mateIn > 0 : (scoreCp ?? 0) >= 0
  ...
  return (
    <div className="evaluation" role="img" aria-label={accessibleLabel}>
      <div className="evaluation__black" style={{ height: `${100 - whiteShare}%` }} />
      {label && (
        <div className={`evaluation__label evaluation__label--${whiteAhead ? 'white' : 'black'}`}>
          {label}
        </div>
      )}
    </div>
